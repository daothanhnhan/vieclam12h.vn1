<?php
    $rows = $acc->getList("book_home_time","","","id","desc",$trang, 20, "book-home");//var_dump($rows);
?>	
            ...
            <tbody>
                <?php 
                    ...
                    foreach ($rows['data'] as $row) {
                        ...
                        $goi = $action->getDetail('bang_gia_3', 'id', $row['home_time_id']);
                    ?>
                        <tr>
                            <td><?= $d ?></td>
                            <td><?= $ntd['company']?></td>
                            ...
                            <td><input type="checkbox" onclick="kich_hoat_home(<?= $row['id'] ?>)" <?= $row['state']==1 ? 'checked' : '' ?> ></td>
                        </tr>
                    <?php
                    }
                ?>
...
<script>
    function kich_hoat_home (id) {
        ...
        var xhttp = new XMLHttpRequest();
          ...
          xhttp.open("GET", "/functions/ajax/kich_hoat_home.php?id="+id, true);
          xhttp.send();
    }
</script>