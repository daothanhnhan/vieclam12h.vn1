<?php 
	...
	function goi_diem ($id) {
		...
		if (isset($_POST['add_goidiem'])) {
			...
			$sql = "UPDATE goi_diem SET diem = $diem, gia = $gia WHERE id = $id";
			$result = mysqli_query($conn_vn, $sql);
			if ($result) {
				echo '<script type="text/javascript">alert(\'Bạn đã sửa được một gói điểm.\')</script>';
			} else {
	...
	goi_diem($_GET['id']);
    $info = $action->getDetail('goi_diem', 'id', $_GET['id']);
?>
...
<form action="" method="post" enctype="multipart/form-data">
    <div class="rowNodeContentPage">
        ...
        <div class="boxNodeContentPage">
            ...
            <input type="text" class="txtNCP1" name="diem" id="diem" onkeyup="money(this)" value="<?= number_format($info['diem']) ?>" required/>
            <p class="titleRightNCP">Giá</p>
            <input type="text" class="txtNCP1" name="gia" id="gia" onkeyup="money(this)" value="<?= number_format($info['gia']) ?>" required/>
