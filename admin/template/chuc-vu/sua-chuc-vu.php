<?php 
	...
	function chuc_vu ($id) {
		...
		if (isset($_POST['add_chucvu'])) {
			...
			$sql = "UPDATE office SET name = '$name' WHERE id = $id";
			$result = mysqli_query($conn_vn, $sql);
			if ($result) {
				echo '<script type="text/javascript">alert(\'Bạn đã sửa được một Chức vụ.\')</script>';
			} else {
	...
	$info = $action->getDetail('office', 'id', $_GET['id']);
?>
